...
- (void)cleanUp
{
    ...
    if (totalCount > maxCount || _totalCost > maxCost)
    {
        ...
        for (id key in keys)
        {
            ...
            if (!_delegateRespondsToShouldEvictObject || [self.delegate cache:(OSCache *)self shouldEvictObject:entry])
            {
                if (_delegateRespondsToWillEvictObject)
                {
                    _currentlyCleaning = YES;
                    [self.delegate cache:(OSCache *)self willEvictObject:entry];
